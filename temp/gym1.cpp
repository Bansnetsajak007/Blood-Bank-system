...
public:
    ...
    void saveToFile(std::ofstream& outFile) const {
        outFile << name << "\t" << age << "\t" << gender << "\t" << feePaid << "\t" << totalDays << "\n";
    }

    static Member loadFromFile(std::ifstream& inFile) {
        ...
        inFile >> feePaid >> totalDays;
        inFile.ignore();
        return Member(name, age, gender, feePaid, totalDays);
    }
};
...
private:
    ...
    const std::string fileName = "members.txt";
...
int main() {
    ...
    do {
        ...
        case 1:
            ...
            gender = getValidString("Enter gender[M/F]: ");
